...
class Pelicula
{
    ...
    {
        $sql = "SELECT peliculas.* FROM peliculas JOIN generos ON peliculas.genero_id = generos.id JOIN directores ON peliculas.director_id = directores.id JOIN actor_pelicula ON peliculas.id = actor_pelicula.pelicula_id JOIN actores ON actor_pelicula.actor_id = actores.id WHERE deleted_at IS NULL";
